...
class CheckSubpBoxes(lal.App):
    ...
    def process_unit(self, unit):
        print(f"Checking {unit.filename}")
        ...
        for sb in unit.root.findall(lal.SubpBody):
            ...
            if first_comment == sb.token_start:
                print_error(f"No box for subprogram `{subp_name}`",
                ...
                if self.args.fix:
                    ...
                    add_content[first_line - 1] = [
                        ...
                        "{}-- {} --".format(indent, subp_name),
                        indent + ('-' * (len(subp_name) + 6)),
                        ''
                    ]
            else:
                first_line = first_comment.sloc_range.start.line
                print_error(f"Malformed box for subprogram `{subp_name}`",
                            Location.from_sloc_range(unit, first_comment.next.sloc_range))
