...
% for i in range(len(_self.matchers) - 1, -1, -1):
    ${exit_label}_${i}:
    % if i > 0 and _self.components_need_inc_ref and _self.matchers[i-1].needs_refcount() and not is_discard(_self.matchers[i-1]):
        % if _self.matchers[i-1].get_type().is_ptr:
