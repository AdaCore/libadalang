...
% for i in range(len(_self.matchers) - 1, -1, -1):
    ${exit_label}_${i}:
    % if (not _self.matchers[i-1].res_needed_by_others) and i > 0 and _self.components_need_inc_ref and _self.matchers[i-1].needs_refcount() and not isinstance(_self.matchers[i-1], Discard):
